...
Deno.serve(async (req) => {
  ...
  const row = {
    ...
    /* 客人是否同意別人使用他的圖案。
       -----------------------------------------------------------
       這是【客人自己的選擇】,所以收前端傳的值是對的 ——
       它不是授權判斷,沒有任何東西因為這個欄位而被允許或拒絕。

       ⚠ 沒傳時預設 true,與畫面上的預設勾選一致。
         但「沒傳」與「他取消勾選」必須分得出來 ——
         所以這裡明確判斷 === false,不要用 !! 或 || 那種寫法:
         allow_reuse: body.allow_reuse || true  永遠是 true。 */
    allow_reuse: body.allow_reuse === false ? false : true,
