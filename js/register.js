...
(function (window, document) {
  ...
  /* ⚠ 尚未對外開放(狀態更新於 2026-08-19)
     -------------------------------------------------------------
     member-auth 已指向主後端【正式站】且驗證通過,擋住的理由換了一個:
     現在每一次送出都是【真實會員 + 真的發簡訊 + 與 ERP 同步】,
     而正式站沒有測試站那條撈驗證碼的診斷路由,錯了不好收拾。
     內部人員實跑一次確認無誤之前,不對外開放。

     擋在這裡而不是只拿掉 login.html 的連結,是因為這頁的網址一旦
     被分享或被搜尋引擎收錄,拿掉連結擋不住任何人。

     內部驗證走 register.html?internal=1(見下方 IS_INTERNAL)。
     驗證通過後把這個常數改成 false 即為對外開放 —— 那是使用者的決定,
     不要順手改。 */
  var NOT_READY = true;
