...
(function (window, document) {
  ...
  /* ⚠ 到門市之後那一段,不要講「報會員編號或手機就能兌換」。
     -----------------------------------------------------------------
     2026-08-27 主後端查證後給的事實:
       · 券以 owner_mid 開得出來,App 票券中心也撈得到
       · 但門市核銷是【掃 qrcode】,而 qrcode 只有在 App 點「兌換」時
         才會產生(那張券的 qrcode_effective_time 目前是 0)
       · 門市端沒有「以手機查券」的路徑,現有的兩條都要客編

     也就是說,一個只用官網、沒裝 App、又還沒綁定門市的收禮人,
     到了櫃檯會出現這種局面:兩個人都看得到那張券存在,
     卻沒有任何辦法把它核銷掉。

     所以這裡改成【不承諾任何機制】,只講他到店該做什麼。
     原本那句「報會員編號或手機就能兌換,不需要另外出示什麼」對
     未綁定的人是假的 —— 而未綁定正是這條路線最常見的樣子。 */
  function storeExtraNote() {
    var bound = !Auth || !Auth.isErpBound || Auth.isErpBound();
    if (bound) {
      return '到店時跟店員說你要兌換這份禮物即可,不需要另外準備什麼。';
    }
    return '⚠ 第一次到樂活門市時,請先請店員協助完成【會員綁定】—— ' +
           '綁定之後這張券才會掛到你的會員編號底下,當場就能兌換。' +
           '直接跟店員說「我要先綁定會員」最快。';
  }
