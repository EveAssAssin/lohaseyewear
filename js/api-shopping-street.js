...
(function (root) {
  ...
  if (!core) {
    throw new Error("[LohasApi.shoppingStreet] core module not loaded");
  }
  const { post } = core;
  ...
  async function getAppointedUnitByCode(opts) {
    if (!opts || (!opts.appointed_unit_code && !opts.category_id)) {
      throw new Error("[LohasApi.shoppingStreet] need appointed_unit_code or category_id");
    }
  ...
  async function login(account, password) {
    if (!account || !password) {
      throw new Error("[LohasApi.shoppingStreet] account and password required");
    }
    return post("sodian", {
      method: "login",
