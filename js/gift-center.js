...
(function (window, document) {
  ...
  function recvCardHtml(g) {
    ...
    if (g.status === 'paid') {
      ...
      /* ⚠ 2026-08-27:拿掉「報你的會員編號或手機就行」。
         -----------------------------------------------------------
         主後端查證後確認:門市核銷是掃 qrcode,而 qrcode 只有在
         App 點「兌換」時才產生;門市端也沒有「以手機查券」的路徑,
         現有的兩條都要客編。

         所以那句話對【尚未綁定門市的會員】是假的 ——
         他報了手機,店員查不到,而兩個人都看得到券在那裡。

         已綁定的人照舊講得明確;未綁定的人先講「要先綁定」,
         那是他到店的第一件事,不是可有可無的補充。 */
      var bound = !Auth || !Auth.isErpBound || Auth.isErpBound();
      actions = '<span class="gf-hint"><i class="fa-solid fa-ticket"></i>' +
                (bound
                  ? '可以兌換了 —— 到任一樂活門市跟店員說要兌換這份禮物即可。' +
                    '你挑的款式與刻圖店員都查得到,不用帶任何東西。'
                  : '可以兌換了。⚠ 第一次到門市時請先請店員協助完成【會員綁定】—— ' +
                    '綁定後這張券才會掛到你的會員編號底下,當場就能兌換。' +
                    '你挑的款式與刻圖店員查得到,不用帶任何東西。') +
                '</span>' +
                storeLinkHtml();
