...
(function (window) {
  ...
  function initGrantCreator() {
    ...
    // 「新增照片」按鈕: 內嵌上傳 (代會員上傳到 gallery_posts, 自動通過)
  ...
  async function uploadPhotosForCreator(files) {
    ...
    if (list) list.insertAdjacentHTML('afterbegin', `<p class="empty-text" id="agUploadingTip" style="padding:10px;font-size:12px;color:var(--lohas-brown)">上傳中 (0/${files.length})…</p>`);
    ...
    for (const file of files) {
      try {
        ...
        const tip = document.getElementById('agUploadingTip');
        if (tip) tip.textContent = `上傳中 (${done}/${files.length})…`;
      } catch (err) {
  ...
  function renderCreatorPhotos(photos) {
    ...
    if (!list) return;
    if (photos.length === 0) {
      list.innerHTML = '<p class="empty-text" style="padding:30px 20px;font-size:12px">尚未上傳照片</p>';
      return;
    }
      ...
      photos.map(p => {
        ...
        return (
          '<div style="position:relative;aspect-ratio:1;background:#f4f1ec;border-radius:6px;overflow:hidden">' +
            ...
            '<button type="button" onclick="window.agDeleteCreatorPhoto(\'' + p.id + '\')" style="position:absolute;top:4px;right:4px;width:24px;height:24px;background:rgba(0,0,0,0.65);color:#fff;border:none;border-radius:50%;cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center" aria-label="刪除">×</button>' +
          '</div>'
        );
      }).join('') +
      '</div>';
  ...
  function prefillCreatorForm(c) {
    ...
    if (shareSec) {
      ...
      if (isVirt && c.member_id) loadCreatorPhotos(c.member_id);
    }
  }
