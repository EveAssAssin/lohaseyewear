...
| ② 尺寸 | **1000 px** | potrace WASM 超過約 1150 px 會拋 `offset is out of bounds`(實測 1024✅ 1100✅ 1200✗)。原本用原生解析度,手機照片 3000px 每張都拋錯然後靜靜降級 |
| ③ 門檻 | **128** | ⚠ **不可以沿用 220。** 這一步的輸入已經是純黑白,灰階只來自縮放內插 —— 那代表「這個像素有多少比例被墨蓋住」,正確切點是中間值。用 220 會讓筆畫全面變胖(實測墨色 26.47% → 27.98%),細節互相黏死 |
