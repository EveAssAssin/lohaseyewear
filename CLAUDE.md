...
裁切後的原圖(原生解析度)
  ...
  → ② 重新取樣到面積 1.20M 像素
  → ③ 再二值化(門檻 150)
  → ④ potrace(turdsize 1),失敗才 fallback imagetracer
```
...
| ② 尺寸 | **面積 1.20M px** | ⚠ 上限是【總像素】不是邊長(實測 1100×1100=1.21M ✅ / 1200×1200=1.44M ✗ / **2000×560=1.12M ✅**)。用長邊當上限對寬圖不公平 —— 正方形吃滿 1.00M,16:9 只用到 0.56M。改用面積預算,並加「失敗就縮小一半重試」|
| ③ 門檻 | **150** | ⚠ **不可以沿用 220**(會胖到細節黏死:墨色 26.47% → 27.98%)。但也【不要用中間值 128】—— 128 數學上對,對細線卻不公平:1 像素寬的線縮小後覆蓋率不到一半就被整條抹掉,而這類細線佔墨色的 22%。把誤差拆成「細節掉了/線變胖了」才看得出來(三隻貓):`128 → 細線保住 90.0%`、`150 → 93.9%`、`170 → 96.4% 但線開始黏`。150 幾乎重現 imagetracer 的手感(93.0%),而線條是 potrace 的平滑曲線 |
| ④ turdsize | **1** | 描圖在 1000px、成品寬 90mm,一像素 0.09mm。turdsize 2 丟掉約 0.13mm 見方以下的形狀,而雷射光點才 0.05mm —— **那個尺寸刻得出來**。三隻貓的貓毛就是這樣被吃掉的(輪廓 1010→689,降到 1 之後回到 1055) |
